...
export default function GeoPredict() {
	...
	async function doGeocode(q: string) {
		...
		try {
			...
			const res = await fetch(
				`${API_BASE}/api/geocode/search?${params.toString()}`,
			);
	...
	function handleSelectPick(el: HTMLSelectElement) {
			...
			{hits.length > 0 && (
				<div className="border rounded-xl p-3">
					...
					<select
						...
						onChange={(e) => handleSelectPick(e.currentTarget)}
						onKeyDown={(e) => {
							if (e.key === "Enter" || e.key === " ") {
								e.preventDefault();
								handleSelectPick(e.currentTarget);
							}
						}}
				...
				<div className="h-[420px] w-full">
					{isClient && markerIcon ? (
						<MapContainer
							...
							scrollWheelZoom
							style={
								{ height: "100%", width: "100%" } as MapContainerProps["style"]
							}
						>
							<TileLayer
				...
				{pred ? (
					<div className="rounded-2xl border border-zinc-200/60 dark:border-zinc-800/60 p-4 bg-white/70 dark:bg-zinc-900/50">
						<div className="text-sm text-zinc-500 mb-1">
							backend: {pred.backend} / {t("today")}: {pred.date_d0} /{" "}
							{t("tomorrow")}: {pred.date_d1}
						</div>
					...
					!loading && (
						<div className="text-sm text-zinc-500">{t("hint_start")}</div>
					)
				)}
			</div>
